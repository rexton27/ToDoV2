...
This project uses **Next.js 16.2.4** with React 19. APIs and conventions may differ from your training data. Read `node_modules/next/dist/docs/` before writing Next.js-specific code.

## Commands

All commands run from `todov2/`:
...
**Stack:** Next.js App Router + InstantDB (`@instantdb/react`) + Tailwind CSS v4 + TypeScript

**Entry point:** `app/page.tsx` — a single `"use client"` component. Auth state from `db.useAuth()` drives the top-level render branch (loading → error → sign-in → app).

**Database client:** `lib/db.ts` exports a single `db` instance. Always import `db` from here; never call `init()` elsewhere or import `@instantdb/react` directly in components.

**Schema and permissions** are local TypeScript files pushed to InstantDB via MCP tools:
- `instant.schema.ts` — entity definitions and links; push with `mcp__instant__push-schema`
- `instant.perms.ts` — CEL-based owner rules; push with `mcp__instant__push-perms`

**Auth:** Magic code flow via `db.auth.sendMagicCode` / `db.auth.signInWithMagicCode`. Sign-out via `db.auth.signOut()`.

**Styling:** Tailwind v4 (PostCSS plugin, not CLI). Design: violet-600 primary, zinc/stone neutrals, dark mode via `dark:` variants.

## Active implementation plan

`PLAN.md` describes the in-progress transformation from a generic todo app into a "Feelings × Time" to-do app. Key decisions:
- Mood options: `energised | overwhelmed | avoiding | calm`
- Time estimates (minutes): `5 | 15 | 30 | 60`
- `tag: "avoid"` marks tasks the user is procrastinating on
- Prioritisation logic: `lib/taskFilter.ts` (to be created)
- Screen flow: `context → tasks → completed`, all owned by an `AppShell` component
- Schema needs two entities (`todos` with new fields + `userContext`) and two owner links
